...
- **Geometry (2315/2310 logical)**: 200 cylinders (plus 3 alternates), 2 surfaces (heads), **4 sectors/track**, **321 words/sector** (first word is sector address; user payload often 320 words). 16 logical blocks × 20 words per sector (Disk Monitor view). 1500 rpm; average rotational delay \~20 ms; read/write \~27.8 µs/word. Seek increments of **2 cylinders**; access time \~`7.5ms*N + 22.5ms` with N even. A full cylinder of 8 sectors can be read/written in \~100 ms once aligned.
...
2. **yew-ui (Rust/Yew to WASM)**
   ...
   - **Views**: Disk Map (cylinder×surface×sector grid, allocation color by FLET/LET; hover shows C/H/S, block, file owner); Head/Timing oscilloscope; 1442 Hopper/Path/Stackers animation; 1403 page buffer preview; Console & DSW status; Activity timeline.
...
3. **Integration adapter (future)**
   ...
   - Bus façade for CPU simulator: memory‑mapped or channel‑oriented IOCC enqueue; IRQ lines; DMA word‐strobe.
...
- **Performance**: 60 fps UI with ≤5 ms per frame processing on mid‑range laptop; can run 1× (timed) or 50× (fast) speed.
...
- **Word rate**: 27.8 µs/word → **\~8.9 ms** to stream 321 words.
- **Seek**: `t = 7.5ms * N_even + 22.5ms settle` (N rounded up to even).
- **Cylinder read**: first sector latency + 8× stream windows ≈ 100 ms. All delays can be disabled (`TimingModel::none()`) for tests.
...
// followed by 200 cyl × 2 heads × 4 sectors × 321 u16 words
```
...
- `AudioService`: WebAudio node graph; seek pitch proportional to `|Δcyl|` and accel; soft “clunk” on settle.
...
- **Linear sector index** for cylinder `c`, head `h ∈ {0,1}`, sector `s ∈ {0..3}`:
